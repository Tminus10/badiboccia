...
<div class="page-head">
  ...
  <nav class="page-head-actions calendar-view-toggle">
    <a class="btn btn-sm <?= $view === 'month' ? 'btn-primary' : 'btn-secondary' ?>" href="<?= h($baseUrl . '?view=month&month=' . $month) ?>">Monat</a>
    <a class="btn btn-sm <?= $view === 'list' ? 'btn-primary' : 'btn-secondary' ?>" href="<?= h($baseUrl . '?view=list') ?>">Liste</a>
  </nav>
</div>
...
<?php else: ?>
  ...
  <nav class="calendar-nav">
    <a class="btn btn-secondary btn-sm" href="<?= h($baseUrl . '?view=month&month=' . $prevMonth) ?>">← Vorheriger Monat</a>
    <h2><?= h($monthNames[(int) $monthStart->format('n') - 1]) ?> <?= h($monthStart->format('Y')) ?></h2>
    <a class="btn btn-secondary btn-sm" href="<?= h($baseUrl . '?view=month&month=' . $nextMonth) ?>">Nächster Monat →</a>
  </nav>
